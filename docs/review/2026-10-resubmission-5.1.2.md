...
2. `curl -s "https://busymate.ai/api/embed-status?assistant=shop-busymate-ai-demo-store&ancestors=https%3A%2F%2Fbusymate-ai-demo-store.myshopify.com%2Chttps%3A%2F%2Fonline-store-web.shopifyapps.com%2Chttps%3A%2F%2Fadmin.shopify.com"`
   ...
   `ancestors=https%3A%2F%2Fevil.example` → `"frameable":false`.
...
- [ ] **Extension version decision.** If step 6 ever shows the toggle on and saved with no
      launcher, create a new extension version with this branch's `assistant.js` (needs the
      Shopify CLI + `shopify app config link`) and release it before resubmitting. A new app
      version is also what activates the `domains/*` webhook subscriptions.
- [ ] **Protected customer data.** Open the draft (Partner Dashboard → API access requests →
      ...
      service, App functionality), Name and Address (Customer service); **deselect Email** (no
      tool uses it — data minimisation); re-read the 16 data-protection answers. The request is
